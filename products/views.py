...
from .models import Order, OrderItem, Product # Убедитесь, что модели импортированы
...
def create_order(request):
    ...
    try:
        ...
        # 2. БЕЗОПАСНАЯ ПОЧТА (fail_silently=True ОБЯЗАТЕЛЕН)
        try:
            send_mail(
                ...
                'raceawm@gmail.com',
                [order.email],
                fail_silently=True # Если Google не ответит, заказ НЕ упадет
            )
        except Exception:
            pass # Игнорируем ошибки почты для стабильности

        return Response({"track_id": order.track_id}, status=201)

    except Exception as e:
        logger.error(f"Ошибка создания заказа: {e}")
        return Response({"error": "Ошибка на сервере. Проверьте данные."}, status=500)
